...
Azure still applies deployment quotas and capacity limits. HTTP 429 is returned with available retry headers; Harbor never replays an inference request after dispatch. The verified `gpt-5.6-sol` deployment uses Azure stored background responses for streaming. If its SSE connection drops after Azure supplies a response ID and sequence number, Harbor resumes that same provider response with Azure's `GET ...?stream=true&starting_after=...` operation inside the original request deadline. The stored response is deleted afterward. Other deployments keep the previous one-POST, non-resumable behavior unless their catalog explicitly enables the capability after equivalent validation. The new provider does not guarantee lower latency. Test your actual workload and inspect Azure's deployment metrics when evaluating performance.
...
This admission policy is process-local. It does not coordinate multiple gateway runtimes, discover account limits, or replay Azure inference. The candidate also applies a [shared queue-to-response deadline](azure-request-policy.md). Upstream statuses and Retry-After headers retain their original values and each explicit request makes one inference POST. Explicitly capable deployments may use same-response recovery GETs as described above. Effective retry settings in the already-running Codex desktop remain unverified. The candidate is staged separately until coordinated activation is recorded.
