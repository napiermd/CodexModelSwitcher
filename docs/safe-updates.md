...
Under the exclusive gateway journal lease, it changes only the runtime owner on the captured turn rows. Route and account bindings, uncertain-delivery flags, request records, and counters remain intact. The candidate rejects inference until the same credentials reproduce the captured configuration under the current or immediately preceding revision contract and its required live route probes pass. The only accepted revision migration removes the generated merged picker catalog from the gateway configuration hash; all source catalogs, provider metadata, and credentials must still match. A durable commit receipt precedes removal of the temporary admission gate and resumption of the paused processes.
