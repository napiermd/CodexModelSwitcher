...
The matrix now adds an eleventh case for the production 30-second queue deadline. It holds two established streams, requires a third request to fail locally with 503 and `Retry-After: 1` without an upstream attempt, and then releases both streams to complete. Reports require measured queue duration, no active request lease left for the expired waiter, unchanged uncertainty, exact attempt counts, and intact held streams before setting `queue_timeout_tested`. Fixture budgets are 50 seconds for held streams, 55 seconds for clients, and 60 seconds for Bifrost network timeouts; the Harbor production policy is unchanged. The local synthetic passthrough run passed all eleven cases and accounted for 34 upstream attempts. Pinned-Bifrost verification of this added case is pending.
