...
The model picker explains that tasks using “Model Harbor selection” follow the chosen Grok or Baseten model on their next turn. The footer names the shared Model Harbor connection. Provider-specific task selections remain explicit; never claim an existing OpenAI task has switched just because the global selection changed.
