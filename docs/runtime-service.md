...
The independent process starts without UI-provided provider keys. A UI session that bootstraps a new service supplies its saved Azure/OpenRouter connections after server authentication. In that session, a changed authenticated boot identity can restore already-loaded credentials without another Keychain read. A UI session that attaches to an existing service loads display metadata only and skips automatic credential synchronization. Reopening the UI after a service-manager restart does not itself restore volatile connections. Use **Manage connection → Restore saved connection** for Azure or OpenRouter, then select a saved model if several are available. The action verifies that exact route before publishing its credential; it does not change catalogs, defaults, or task settings. The running gateway must match the app. Existing tracked turns retain their original credential-binding and uncertain-delivery checks; untracked admissions prevent restoration. A pending runtime update requires coordinated maintenance first. See the [restore contract and checks](verification/provider-restoration.md). Durable credential bootstrap while the UI is closed remains unimplemented.
