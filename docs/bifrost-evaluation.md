...
The direct Azure branch now holds a process-local admission permit through upstream close and uses one absolute 180-second request deadline, including queueing and network work. Verification uses ten seconds. Azure makes one upstream attempt and retains uncertain task ownership after interrupted delivery. Baseten's pacing remains separate. The composed synthetic study below tests these controls with pinned Bifrost and a fixture upstream. Real Azure parity remains unverified. See [the complete policy and limits](azure-request-policy.md).
