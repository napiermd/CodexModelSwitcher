...
The direct Azure branch now holds a process-local admission permit through upstream close and uses one absolute 180-second request deadline, including queueing and network work. Verification uses ten seconds. Azure makes one inference POST. Explicitly capable deployments can resume the same stored response with provider GETs; this does not replay inference. Harbor retains uncertain task ownership when interrupted delivery cannot be recovered. Baseten's pacing remains separate. The composed synthetic study below tests the original controls with pinned Bifrost and a fixture upstream. Real Azure parity remains separately verified. See [the complete policy and limits](azure-request-policy.md).
...
2. Synthetic invalid authentication, three concurrent workers, opaque event forms, and token-count fidelity now pass. Complete coverage against actual Codex request/event fixtures and monetary cost provenance. Bifrost's own active-stream limiting is disproved. Harbor's process-local stream admission passes the composed synthetic study below; multiple-runtime admission remains unverified. Verify immutable pin provenance in the eventual deployment process.
...
The first ten-case matrix passed on both native architectures in Actions run `35299527911` at source `e82b9ba`. All 32 upstream attempts per architecture were accounted for. Both reports verified the source inventory digest `653756631b471200ce4fc94e3738ab20baa9defc5ff289e07347092afc07d647`, entrypoint identity, cleanup, unchanged source, and content-free logs. Those reports correctly leave queue expiration and live Azure tests unperformed.

The matrix now adds an eleventh case for the production 30-second queue deadline. It holds two established streams, requires a third request to fail locally with 503 and `Retry-After: 1` without an upstream attempt, and then releases both streams to complete. Reports require measured queue duration, no active request lease left for the expired waiter, unchanged uncertainty, exact attempt counts, and intact held streams before setting `queue_timeout_tested`. Fixture budgets are 50 seconds for held streams, 55 seconds for clients, and 60 seconds for Bifrost network timeouts; the Harbor production policy is unchanged. The local synthetic passthrough run passed all eleven cases and accounted for 34 upstream attempts. Actions run `35301121482` at source `dbc1e61` then passed all eleven cases on both pinned architectures. Queue expiration took 30,017.81 ms on AMD64 and 30,018.87 ms on ARM64. Both reports observed held-stream/expired-waiter attempt counts of 1/1/0, successful stream completion, 34 accounted-for upstream attempts, and complete cleanup. [Selected report fields and original report hashes](verification/composed-queue-expiration.json) preserve the result and actual merge-checkout provenance.
