...
def owner_request(method, path, body, port, token_path):
    ...
    try:
        ...
        connection.settimeout(COMMAND_TIMEOUTS[(method, path)])
...
def main():
    try:
        ...
        if not isinstance(command, dict) or set(command) != {'method', 'path', 'body_base64'}:
        ...
        result, runtime = owner_request(command['method'], command['path'], body,
            int(os.environ.get('MODEL_HARBOR_PORT', '48118')), token)
