...
5. Reasoning replay table: `ModelHarbor/Support/chat_reasoning.py`, evidence-backed family matching, passthrough for unlisted models. The isolated Chat compatibility pilot now consumes the table in both directions: it restores prior plaintext reasoning as `reasoning_content` on the assistant turn and maps returned `reasoning_content` back to non-visible Responses reasoning items. Unlisted models and encrypted reasoning fail closed. Production Baseten routing still uses `/responses`; Harbor has no live Baseten Chat route to enable yet.
...
10. Tool-result aging estimator: `tool_result_aging.py` computes reclaimable bytes with the reference eligibility rules; pure functions, no mutation. `scripts/audit-context.py --tool-result-aging` samples explicit rollout history at compaction boundaries and at the latest history. Reports contain counts, byte estimates, item indexes, and SHA-256 digests, but no content or call IDs. The mode is advisory and does not rewrite history.

Final verification after the wiring, CLI, and packaging corrections: 552 Python tests pass and 108 Swift tests pass. A clean Xcode build succeeds, strict deep signature verification passes, and the app bundle contains no test-generated Python bytecode. One earlier regression was caught during integration (missing `ssl` import in the new failure handler) and fixed before the final runs.
