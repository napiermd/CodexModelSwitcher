...
`--cases basic stream image` selects a smaller diagnostic run. The complete current 24-case suite is required for any full-suite claim. Historical result files retain their original 17-case matrix and provenance. The AMD64 digest is recorded but was not exercised in the September 17 run.
...
Twenty-seven hermetic tests cover the driver and protocol assertions. The September 17 native-surface container attempt failed during `docker start`, before any test request; `results.passthrough-startup-failed.json` records that failure and the cleanup follow-up. It supplies no native transport pass or latency evidence.
